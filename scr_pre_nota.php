<? // Analizar 22/09/07 recebo solicitação para gerar prenotas para orçamento cuja COD_DECISAO > 1 E FECHAMENTO É NULO NÃO RECEBE MENHUMA VARIAVEL.

// Script ativado pela Página frm_orc_definir....
// Este script é o responsável por atribuir a cada 10 itens um numero de pre_nota que gera um relatório para o dpt. de contas
...
$sql="select cod_cp from orc where fechamento = $fechamento and cod_decisao = 0";
$res=mysql_query($sql) or die ("$sql").mysql_error();
$row=mysql_num_rows($res);
if($row>0){
	$cp=mysql_result($res,0,"cod_cp");
	die("<h1>Erro Grave!!! Por algum motivo exite(m) $row item(ns) orçado(s) neste fechamento indefinido(s). 
	<br>Verifique e aprove ou reprove antes de gerar a pré-notas.<br>
	cp");
...
while($linha=mysql_fetch_array($res)){
	...
	where orc.fechamento = $fechamento and cod_peca=$linha[cod_peca] and orc_decisao.aprova=1 and orc.valor like $linha[valor]";
	
	mysql_query($sql2) or die ("$sql2<br>".mysql_error());
	$rowGravados=mysql_affected_rows();
	if ($rowGravados==0){
		die("Nenhum registro afetado <br>".$sql2);
		//ATENÇÃO USAR O SQL ABAIXO PARA LimpaR as colunas de orc para desfazer UM PROCESSO INTEIRO DE FECHAMENTO E PRENOTAS GERADAS

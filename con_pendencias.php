...
//
$sqlorcAgAp20=mysql_query("SELECT cp.cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod 
WHERE data_pronto IS NULL AND data_analize IS NOT NULL and cod_decisao=0  AND (DATEDIFF(now( ) , data_entra) <=20) GROUP BY cp.cod")or die(mysql_error());
$lorcAgAp20=mysql_num_rows($sqlorcAgAp20);

$sqlorcAgAp2030=mysql_query("
SELECT cp.cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod 
WHERE data_pronto IS NULL AND data_analize IS NOT NULL and cod_decisao=0 AND (DATEDIFF(now( ) , data_entra) >20) AND (DATEDIFF(now( ) , data_entra) <30) GROUP BY cp.cod")or die(mysql_error());
$lorcAgAp2030=mysql_num_rows($sqlorcAgAp2030);

$sqlorcAgAp30=mysql_query("SELECT cp.cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod 
WHERE data_pronto IS NULL AND data_analize IS NOT NULL and cod_decisao=0  AND (DATEDIFF(now( ) , data_entra) >=30) GROUP BY cp.cod")or die(mysql_error());
$lorcAgAp30=mysql_num_rows($sqlorcAgAp30);

$sqlorcAp20=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND aprova =1 AND (DATEDIFF(now( ) , data_entra) <=20) GROUP BY cod")or die(mysql_error());
$lorcAp20=mysql_num_rows($sqlorcAp20);

$sqlorcAp2030=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND aprova =1 AND (DATEDIFF(now( ) , data_entra) >20) AND (DATEDIFF(now( ) , data_entra) <30) GROUP BY cod")or die(mysql_error());
$lorcAp2030=mysql_num_rows($sqlorcAp2030);

$sqlorcAp30=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND aprova =1 AND (DATEDIFF(now( ) , data_entra) >=30) GROUP BY cod")or die(mysql_error());
$lorcAp30=mysql_num_rows($sqlorcAp30);
//
$sqlorcRp20=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND reprova =1 AND (DATEDIFF(now( ) , data_entra) <=20) GROUP BY cod")or die(mysql_error());
$lorcRp20=mysql_num_rows($sqlorcRp20);

$sqlorcRp2030=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND reprova =1 AND (DATEDIFF(now( ) , data_entra) >20) AND (DATEDIFF(now( ) , data_entra) <30) GROUP BY cod")or die(mysql_error());
$lorcRp2030=mysql_num_rows($sqlorcRp2030);

$sqlorcRp30=mysql_query("SELECT cp.cod AS cod FROM cp INNER JOIN orc ON orc.cod_cp = cp.cod INNER JOIN orc_decisao ON orc_decisao.cod = orc.cod_decisao
WHERE data_pronto IS NULL AND data_analize IS NOT NULL AND reprova =1 AND (DATEDIFF(now( ) , data_entra) >=30) GROUP BY cod")or die(mysql_error());
...
<td><? print($p30pronto);?></td>
        <tr bgcolor="#CCCCCC">
          <td><div align="center"><strong> <a href="con_pendenciatecgeral.php" title="Mostra peças necessárias para liberação destes produtos!" target="mainFrame" class="style5">Aguardando Pe&ccedil;as</a></strong></div></td>
          <td class="Cabe&ccedil;alho"><div align="center"><span class="style9"><?print ($ltec)?></span></div></td>
		  <td rowspan="4" class="Cabe&ccedil;alho">
            <div align="center"><a href="con_pendenciatecgeral.php" title="Mostra todos os produtos parados na area técnica!" target="mainFrame" class="style5">
            <?
...
?>
        ...
        <tr bgcolor="#CCCCCC">
          <td><div align="center" class="style5"><a href="con_pendenciatecgeral.php" title="Mostra todos os produtos em orçamento!" target="mainFrame" class="style5"><strong>Or&ccedil;amentos Aguardando Aprova&ccedil;&atilde;o </strong></a></div></td>
        ...
        <tr bgcolor="#CCCCCC">
          <td><div align="center" class="style5"><a href="con_pendenciatecgeral.php" title="Mostra todos os produtos em orçamento!" target="mainFrame" class="style5"><strong>Or&ccedil;amentos Aprovados </strong></a></div></td>
        ...
        <tr bgcolor="#CCCCCC">
          <td><div align="center" class="style5"><a href="con_pendenciatecgeral.php" title="Mostra todos os produtos em orçamento!" target="mainFrame" class="style5"><strong>Or&ccedil;amentos Reprovados </strong></a></div></td>
